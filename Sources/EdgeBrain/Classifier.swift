...
public struct Classifier {
  ...
  public var inputIDs: [(Int, Int)] = []
  public var outputIDs: [Int] = []

  public init(inputCount: Int, labelCount: Int, hiddenCount: Int) {
    program = Program()
    for _ in 0..<inputCount {
      self.inputIDs.append((program.addNode(kind: .input), program.addNode(kind: .input)))
    }
  ...
  public func classify(features: [Bool]) -> [Float] {
    ...
    let outCounts = program.run(withInputs: zip(inputIDs, features).map { $0.1 ? $0.0.1 : $0.0.0 })
      .outputs
    let outLogits = outputIDs.map { Float(outCounts[$0]!) }
    let logitMax = outLogits.max()!
    let unscaledResult = outLogits.map { exp($0 - logitMax) }
    let normalizer = log(outLogits.reduce(0, +))
    return unscaledResult.map { $0 - normalizer }
  }
